// ============================================================
//  RCReceiver.cpp  –  FlySky PWM receiver  (5 channels)
...
// ----------------------------------------------------------------
// Global instance + one ISR wrapper per channel
// ----------------------------------------------------------------
...
void RCReceiver::begin()
{
    ...
    Serial.println("[RC] FlySky receiver initialised OK");
}
...
// ----------------------------------------------------------------
// update()  –  consume ISR data, validate, normalise
// ----------------------------------------------------------------
void RCReceiver::update()
{
    ...
    for (uint8_t i = 0; i < RC_NUM_CHANNELS; i++) {
        ...
        _newData[i]    = false;  // consume
        interrupts();

        if (fresh) {
            _data.pulseUs[i] = pulse;
            // Throttle (ch1) clamped to [0, +1], rudder and switches [-1, +1]
